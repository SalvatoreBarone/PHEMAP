...
typedef enum PHEMAP_PROTOCOL_MESSAGE_TYPE_T
{
	...
	verification_reply = 6, /**< Reply for a verification request */

	salt_request = 7, /**< Salted PHEMAP carnet request, sent by D to AS */

	salt_reply = 8, /**< Salted PHEMAP reply, sent by AS to D */

	salt_ack = 9, /**< Salted PHEAMP ack, sent by D to AS, while entering in
					   salted mode */

	salted_carnet = 10, /**< Salted PHEMAP carnet. Sent by AS to G */

	salt_invalidation = 11, /**< Salt invalidation message, sent bu D to AS */

	babelchain_setup_request = 12, /**< Babel-chain PHEMAP setup request, sent 
									    by D to the AS. */

	babelchain_carnet = 13, /**< Babel-chain PHEMAP carnet, sent by AS to D */

	application = 14 /**< Application message, built on top of PHEMAP */
}
...
typedef struct PHEMAP_PROTOCOL_MESSAGE
{
	...
	union 
	{
		...
		PHEMAP_Carnet_t carnet; /**< A PHEMAP salted carnet
